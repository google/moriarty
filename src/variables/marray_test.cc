...
TEST(MArrayTest, LengthIsSatisfied) {
  ...
  for (int tries = 0; tries < 1000; tries++) {
    EXPECT_THAT(Generate(MArray(MInteger()).OfLength(tries / 2, tries)),
                IsOkAndHolds(SizeIs(AllOf(Ge(tries / 2), Le(tries)))));
  }
}
...
TEST(MArrayTest, LengthPropertyIsProcessedProperly) {
  MArray<MInteger> arr =
      MArray(MInteger()).OfLength(MInteger().Between(1, 1000));
