...
TEST(OStringStream, IsOStream) {
  static_assert(std::is_base_of<std::ostream,
                                moriarty::moriarty_internal::OStringStream>(),
                "");
}

TEST(OStringStream, ConstructNullptr) {
  moriarty::moriarty_internal::OStringStream strm(nullptr);
...
TEST(OStringStream, ConstructStr) {
  std::string s = "abc";
  {
    moriarty::moriarty_internal::OStringStream strm(&s);
...
TEST(OStringStream, Destroy) {
  ...
  moriarty::moriarty_internal::OStringStream strm(s.get());
  s.reset();
}
...
TEST(OStringStream, MoveConstruct) {
  std::string s = "abc";
  {
    moriarty::moriarty_internal::OStringStream strm1(&s);
    strm1 << std::hex << 16;
    EXPECT_EQ(&s, strm1.str());
    moriarty::moriarty_internal::OStringStream strm2(std::move(strm1));
...
TEST(OStringStream, MoveAssign) {
  std::string s = "abc";
  {
    ...
    strm1 << std::hex << 16;
    EXPECT_EQ(&s, strm1.str());
    moriarty::moriarty_internal::OStringStream strm2(nullptr);
...
TEST(OStringStream, Str) {
  std::string s1;
  moriarty::moriarty_internal::OStringStream strm(&s1);
  const moriarty::moriarty_internal::OStringStream& c_strm(strm);
...
TEST(OStreamStream, WriteToRValue) {
  std::string s = "abc";
  moriarty::moriarty_internal::OStringStream(&s) << "";
  EXPECT_EQ("abc", s);
  moriarty::moriarty_internal::OStringStream(&s) << 42;
  EXPECT_EQ("abc42", s);
  moriarty::moriarty_internal::OStringStream(&s) << 'x' << 'y';
  EXPECT_EQ("abc42xy", s);
}
